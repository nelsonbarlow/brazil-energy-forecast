...
\IEEEauthorblockA{Centro de Inform\'atica --- Universidade Federal de Pernambuco\\
...
\textbf{The same covariate hurts a trained model.} A natural control: does the holiday
flag help the locally-trained N-BEATS as much? We add the identical feature to the tuned
N-BEATS (3 seeds). It does the \emph{opposite}---overall
MAPE worsens from 1.91\,\% to 2.20\,\% ($+15$\,\%) and holiday MAPE from 6.00\,\% to
6.30\,\% ($+5$\,\%). The interpretation sharpens the universality boundary: N-BEATS, trained
on 5+ years of local data, has \emph{already} absorbed Brazilian holidays implicitly into
its weights, so an explicit flag is redundant and merely adds noise; the zero-shot model
never saw those holidays and gains from being told. The same intervention helping the
foundation model and hurting the trained one is direct evidence that what the foundation
model lacks---and the flag supplies---is precisely \emph{local calendar knowledge}.
...
ONS data: dados.ons.org.br (CC-BY 4.0). TEPCO data: tepco.co.jp. All code and result tables
are released for reproducibility; the project README maps each section, table, and figure of
this paper to the exact command and result file that produces it. Hardware: Mac Mini M4, 24\,GB,
no GPU.
